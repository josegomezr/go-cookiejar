// _ Copied from the go source because I'm too lazy to reimplement everything.
...
import (
	...
	"errors"
	"fmt"
	"github.com/josegomezr/go-cookiejar/internal/ascii"
	"net"
	"net/http"
	orig_cookiejar "net/http/cookiejar"
	"net/url"
	"slices"
	"strings"
	"sync"
...
type Jar struct {
	psList orig_cookiejar.PublicSuffixList
...
// Options.
func New(o *orig_cookiejar.Options) (*Jar, error) {
	jar := &Jar{
...
func jarKey(host string, psl orig_cookiejar.PublicSuffixList) string {
	if isIP(host) {
		return host
	}
